<template>
	<div id="bProductDetails">
		...
		<div class="nav">
			<span @click="goto(1)">商品简述</span><span>|</span>
			<span @click="goto(2)">报价信息</span><span>|</span>
			<span @click="goto(3)">产品详情</span>
		</div>
		<div class="content">
			<div class="contentsp listshow1" style="padding-top: 2rem;">
				...
				<div class="spbottom">
					<img src="../../assets/img/bproduct/cpxq1.jpg" style="width: 100%; height: 14rem;" />
					<p class="p1">{{productDetails.name}}</p>
					<p><span class="s1">材质：<span>{{productDetails.productMaterial}}</span></span><span>颜色：<span>{{productDetails.productMaterial}}</span></span>
					</p>
					<p><span class="s1">类别：<span>{{productDetails.categoryName}}</span></span><span>风格：<span>{{productDetails.series}}</span></span> </p>
				</div>
			...
			<div class="contentbj listshow2" >
				...
				<img :src="productDetails.wxOfferFiles.fullPath" style="width: 100%; height: 21.5rem;" />
			</div>
			<div class="contentxq listshow3"  style="padding: 2rem 0;" >
				<div class="sptop">
				...
				<img v-for="(chanpin,index ) in chanpinimg" :key='index' :src="chanpin.fullPath" style="width: 100%; " />
			</div>
		</div>
		<!-- <div class="btn">
			<button style="box-shadow:0px 0px 8px 1px rgba(218,218,218,1);
			height: 2.5rem;">取消</button><button style="background:rgba(187,174,135,1); color: white;">推荐</button>
		</div> -->
	</div>
</template>
	...
	export default {
		...
		mounted: function() {
			...
			this.axios({
				...
				url: '/wxapi/b/products/' + productId
