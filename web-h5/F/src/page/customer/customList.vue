			  ...
			  <ul class="custom_select">
				   <li>
					   <div style="width:100%;  position: relative;">
						   <input :readonly="onlyread" style="width: 100%; text-align: center; height: 1.4rem; margin-top: 0.25rem ;  font-size: 0.625rem;  border: none; outline: none; color: #000000;" @change="searchdata()"  v-model="searchObj.cityIdName" @click="addsiteshow=!addsiteshow" id="xzdq" placeholder="选择地区">
						   <span style="position:absolute;top:0.3rem; left:90%; width: 0.4rem; height: 0.4rem;"><img style="width: 100%;" src="../../assets/img/custom/jiantou.png"/></span>
					    </div>
				   </li>
				   <li  style="text-align: center;">
					   ...
					       <select class="mui-btn mui-btn-block"  style="  color: #000000;font-size: 0.625rem; padding: 0; margin-top: 0.55rem; margin-left: 1.4rem;" v-model="search_dealer"  @change="getvaluedealer($event)">
						        <option  readonly  value="">经销商</option>
						   	    <option :key='index' :value="coupon.companyId" v-for="(coupon,index) in dealerList" >{{coupon.companyName}}</option>
						   </select>
				   ...
				   <li  style="text-align: center;">
					   ...
					       <select class="mui-btn mui-btn-block"  style="color: #000000;font-size: 0.625rem; margin-left: 0.6rem; padding: 0;margin-top: 0.55rem;" v-model="search_type"  @change="getcustom($event)">
						     <option readonly value="">客户类型</option>
						   	 <option :key='index' :value="coupon.value" v-for="(coupon,index) in couponList2" >{{coupon.name}}</option>
						   </select>
					       <span style="position:absolute;top:-0.3rem; left:66%; width: 0.4rem; height: 0.4rem;"><img style="width: 100%;" src="../../assets/img/custom/jiantou.png"/></span>     
					   </div>
				   </li>
			   ...
			   <div v-show="customLists.length==0">
	   		   ...
	   		   <div v-show="customLists.length>0">
					  <ul class="mui-table-view" style="background-color: white; border-radius:0.25rem;padding:0.5rem ">
							<li class="mui-table-view-cell" v-for="item in customLists" @click="changePath(item.customerId)">
									...
									<div>
										...
										<div style="float: left; width: 84%;">
											<ul >
												...
												<li style="font-size: 0.6rem; margin-left: 0.5rem; color:rgba(99,99,99,0.5); ">{{item.customerMergerName}}</li>
											</ul>
										</div>
										<div class="clearfix"></div>
									</div>
	...
	export default {
		...
		methods: {
			...
			changePath:function  (customerId) {
				...
				this.$router.push({ name: 'customDep', query: { customerId:customerId }})
