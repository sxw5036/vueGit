			...
			<div class="">
				...
				<div class="linkman_nav">
					...
					<ul class="linkman_lsit">
						<li class="linkman_item" v-for="(item,index) in dataList " :key='index'>
							
							<!-- 	<router-link :to="{name:'FchatBoard',query:{branchId:item.branchId}}"> -->



									<div class="portrait" @click="gotoChatBoard(item.branchId)">
										<img src="../../assets/music.png" />
									</div>

									<div class="user_msg"   @click="gotoChatBoard(item.branchId)">
										<h3>{{item.name}}<span>{{item.leaderTel}}</span></h3>
										<div class="notes">
											<p>{{item.note}}</p>
										</div>
									</div>
...
// 	 */
	...
	export default {
		...
		methods: {
			gotoChatBoard(){
				this.$router.push({name: 'FchatBoard',query:{branchId:branchId}})
			},
			inputFocus: function() {
		...
		mounted: function() {
			...
			that.axios({
				...
				url: '/wxapi/f/companyMessages/companyList'
