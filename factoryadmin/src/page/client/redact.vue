	...
	<div>
		<div class="Breadcrumb">
			<Breadcrumb>
				...
				<BreadcrumbItem>
					<a href="javascript:history.back(-1)">客户管理</a>
				</BreadcrumbItem>
			...
			<div class="iv_formbox">
				...
				<div class="iv_form">
					<Form :model="objmsg" :label-width="120">
						...
						<FormItem label="客户电话">
							<Input v-model="objmsg.phone" placeholder="请输入客户联系方式..."></Input>
						</FormItem>
						...
						<!--<FormItem label="经销商">
							...
							<Select filterable v-model="objmsg.companyId" @on-change="changedistric(districMsg)">
								<Option v-for="(item,index) in companyData" :value="item.id" :key="item.id">{{ item.name }}-{{ item.founderName }}</Option>
							</Select>
...
<script>
	...
	export default {
		...
		mounted: function() {
			...
			this.path(this.$route.query.item)

			//			this.axios({
			//				method: 'get',
			//				url: '/api/f/dealers/companies?state=1&pageNum=1&pageSize=99999',
			//
			//			}).then(function(res) {
			//
