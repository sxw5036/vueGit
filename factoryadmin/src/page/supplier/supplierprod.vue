	...
	<div>
		<div class="Breadcrumb">
			<Breadcrumb>
				...
				<BreadcrumbItem>供应商产品列表</BreadcrumbItem>
			</Breadcrumb>
		</div>
		...
		<!--		<div class="operation_box">
			<div class="active">
				...
				<div class="ui_operateLeft" style="line-height: 32px;">
					...
					<input class="iv_input"  onkeyup="this.value=this.value.replace(/[^\u4e00-\u9fa5\w]/g,'')"  id="p_lea" v-model="searchObj.name" placeholder="请输入公司名称..." />
					<span class="operation_span">联系电话：</span>
					<input class="iv_input"  onkeyup="this.value=this.value.replace(/[^\u4e00-\u9fa5\w]/g,'')"  id="p_lea" v-model="searchObj.leaderTel" placeholder="请输入联系电话..." />

					<Button shape="circle" icon="ios-search" @click="clicksearch"></Button>
				</div>

				<div class="ui_operateRight">
					<Button @click="newData" class="p_lea" shape="circle" icon="ios-refresh"></Button>
					<Button type="primary" @click="GetModelOpen"  v-show="operation.edit==true">添加供应商</Button>
		...
		<div class="supplierprod">
           ...
           <div class="operation_box">
			...
			<div>
				<!--supplier-->
				<Table border :columns="setsupplierHead" :height="500" :data="setsupplierData" ref="table"></Table>
			</div>
...
//					/that.productShow = true
			...
			getDatas: function() {
				...
				this.axios({
					method: 'get',
					url: '/api/f/suppliers?pageNum=' + this.pageIndex + '&pageSize=10000',
					...
					that.tableData = data
					that.setsupplierOpen(that.SetIndex)
					that.pageIndextotal = dataAll.pagination.total
					that.loading = false
